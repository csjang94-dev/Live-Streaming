...
# ==================== CloudFront Outputs ====================
output "player_cloudfront_url" {
  description = "웹 플레이어 CloudFront URL (HTTPS)"
  value       = "https://${aws_cloudfront_distribution.player.domain_name}"
}

output "hls_cloudfront_url" {
  description = "HLS CloudFront URL (HTTPS)"
  value       = "https://${aws_cloudfront_distribution.hls.domain_name}/live.m3u8"
}

output "player_s3_url" {
  description = "웹 플레이어 S3 Website URL (HTTP)"
  value       = "http://${module.storage.player_website_endpoint}"
}
